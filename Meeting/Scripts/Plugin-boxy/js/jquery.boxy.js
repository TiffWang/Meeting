...
jQuery.extend(Boxy, {
    ...
    //obj是按钮本身。如： onclick="return Boxy.sConfirm(this,'确定删除嘛？',null);"
    sConfirm: function (obj, message, options) {
        Boxy.ask(message, ['确认', '取消'], function (response) {
            if (response == '确认') { $(obj).attr("onclick", "").click(); }
        }, options);
        return false;
    },
    ...
    ask: function (question, answers, callback, options) {
        ...
        if (options.isConfirm) {
            jQuery(document.body).bind('keyup.boxy', function (evt) {
                var key = evt.which || evt.keyCode;
                if (key == 13) {//13回车
                    jQuery("input[value='确认']", buttons).click();
                    jQuery(document.body).unbind('keyup.boxy');
                }
            });
        }
